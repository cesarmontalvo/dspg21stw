...
```{r}
d$PER_CHG <- as.numeric(d$PER_CHG)
```
...
```{r, fig.height = 20}
ggplot(d, aes(PER_CHG)) + geom_histogram() + facet_wrap(~SOC_type)
```
...
```{r, fig.height = 20, fig.width = 20}
E(stw_network)$color <- colour_values(E(stw_network)$PER_CHG, palette = "magma")
plot(stw_network,
     vertex.label = NA,
     vertex.label.color = "black",
     vertex.label.cex = 0.5,
...
E(arch_net)$color <- colour_values(E(arch_net)$PER_CHG, palette = "viridis")
plot(arch_net,

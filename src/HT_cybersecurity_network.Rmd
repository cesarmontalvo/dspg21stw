...
```{r}
rolelab <- ifelse(V(g_test)$type == F, V(g_test)$name, NA)
```

```{r}
rolelab[!is.na(rolelab)]
```
...
rolecat <- as.factor(V(g_test)$STW)
...
plot(g_test,vertex.shape=ifelse(V(g_test)$type,"circle","square"),
     ...
     edge.color = ifelse(E(g_test)$STW == "STW", cbPalette.blk[7], cbPalette.blk[1]),
     vertex.label = ifelse(V(g_test)$type == F, V(g_test)$name, NA),
     vertex.label.cex = 0.7,
     vertex.label.dist = 0,
     vertex.label.color = "black",
     vertex.size = ifelse(V(g_test)$type, 2, 5),
     directed = F)
